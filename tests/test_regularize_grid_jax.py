...
def apply_normalization(
    ...
    num_electron = integrate_density_with_weights(
        jnp.asarray(grid_weights), jnp.asarray(unscaled_coefficient_inputs)
    )
    factor_electron = integrate_density_with_weights(
        jnp.asarray(downsampled_grid_weights), jnp.asarray(downsampled_charge_density)
    )

    tot_volume = jnp.sum(jnp.asarray(grid_weights))
    factor_volume = jnp.sum(jnp.asarray(downsampled_grid_weights))

    normalized_grid_weights = jnp.asarray(downsampled_grid_weights) * (
...
def test_normalization_invariants(use_reg2):
    ...
    interpolated_charge_density = regularizer(
